...
const PlayersTable = (props) => {
  ...
  return (
    <>
      {!(players.pageResults.length > 0) && (
        <h2 className="errorMsg">No Players Found!</h2>
      )}
          ...
          >
            <TableHead>
              <TableRow sx={{ cursor: "default" }}>
                ...
                <TableCell className="ageColumn" align="center">
                  <h4>Birthday (Age)</h4>
                </TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {players?.pageResults.map((p) => {
                let playerNameSplit = p.playerPerson.name.split(" ");
                  ...
                  >
                    <TableCell component="th" scope="row">
                      <div className="flex flex-ai-c">
                        <img
                          className="playerImg"
                          src={p.playerPerson?.image}
                          alt="person"
                        />
                        <div>
                          <h4 className="teamPlayersTablePlayerName">
                            {playerName}
                          </h4>
                          <p className="teamPlayersTablePlayerPosition">
                            {p.position}
                          </p>
                        </div>
                      </div>
                    ...
                    <RatingTableCell
                      overall
                      rating={p.playerStats?.overallRating}
                    />
                    ...
                    {isPlayersPage && (
                      <>
                        <TableCell align="center">
                          <img
                            className="teamLogo"
                            src={p.currentTeam?.teamLogo}
                            alt="person"
                          />
                        </TableCell>
                        <RatingTableCell
                          hide
                          rating={p.playerStats?.attacking}
                        />
                        <RatingTableCell
                          hide
                          rating={p.playerStats?.playMaking}
                        />
                        <RatingTableCell
                          hide
                          rating={p.playerStats?.defending}
                        />
                        <RatingTableCell
                          hide
                          rating={p.playerStats?.goalkeeping}
                        />
                      </>
                    )}
                    ...
                    <TableCell align="center">
                      {getDate(p.playerPerson?.birthDate)}
                    </TableCell>
                  </TableRow>
                );
              })}
