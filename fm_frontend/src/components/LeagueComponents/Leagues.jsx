...
const Leagues = () => {
  ...
  useEffect(() => {
    if (user == null) {
      ...
      GeneralAxiosService.getMethod(
        "https://localhost:7067/api/v1/Leagues/" + params.id
      )
        .then((res) => {
          setLeague(res.data);
        })
  ...
  return (
    <section className="leagueSection container container--pa">
      <h1 className="title">League Details</h1>
      {loading && <Loading />}
      {!loading && (
        <div className="leaugeHeader container container--styled">
          <LeagueHeader league={league} image={league.leagueLogo} />
          <HeaderTabs
