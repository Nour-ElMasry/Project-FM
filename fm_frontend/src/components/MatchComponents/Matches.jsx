...
import randomizeList from "../../services/HelperMethods";
...
const Matches = () => {
  ...
  var groupByLeagueName = (list) => {
    return list.reduce((a, b) => {
      (a[b["fixtureLeague"].leagueName] =
        a[b["fixtureLeague"].leagueName] || []).push(b);
      return a;
    }, {});
  };
  ...
  useEffect(() => {
    if (user == null) {
      ...
      GeneralAxiosService.getMethod(
        "https://localhost:7067/api/v1/Fixtures/All/" + pageApi
      )
        .then((res) => {
          setApiData(res.data);
          setMatches(groupByLeagueName(randomizeList(res.data.pageResults)));
        })
        ...
        <>
          <div className="gameweekTables">
            {_.map(matches, (x, i) => {
              return (
                <MatchesTable
                  tableTitle={x[0].fixtureLeague.leagueName}
                  fixtures={x}
                  key={i}
                />
              );
  ...
  return (
    <section className="matchSection container container--pa">
      <h1 className="title">Matches From All Leagues</h1>
      {!hasError && (
        <Pagination
          gameweekPagination={true}
          page={pageApi}
          maxPages={apiData.totalPages}
          pageLoading={loading}
